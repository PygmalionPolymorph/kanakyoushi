...
Helpers.readStatisticsFromDB = function() {
  if (Meteor.user()) {
    if (Statistics.find({userId: Meteor.user()._id}).count() == 0) {
      ...
      return {"userId": Meteor.user()._id, "totalTries": 0, "detail": {}};
    } else {
...
Helpers.createEmptyStatisticsForUser = function() {
  var statistics = {"userId": Meteor.user()._id, "totalTries": 0, "detail": {}};
  var symbols = Symbols.find().fetch();
  for (var i = 0; i < symbols.length; i++) {
    statistics["detail"][symbols[i]._id] = {"hiragana": {"tries": 0, "correct": 0}, "katakana": {"tries": 0, "correct": 0}};
  }
